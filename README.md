...
1. Сборка образа:
```bash
docker build -t fast-noise-app .
```

Запуск контейнера:
```bash
docker run -p 8001:8001 fast-noise-app
```
...
ссылка для примера
```bash
http://127.0.0.1:8001/generate?w=1000&h=1000
```
w: ширина (пиксели)
...
Bash
```bash
curl -o /dev/null "http://127.0.0.1:8001/generate?w=20000&h=20000"
```
